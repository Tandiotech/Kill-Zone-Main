...
const SWING = 1.15; // max sun angle (rad, ~66°) from the camera axis — keeps the visible face lit
...
export async function globe(host, {
  ...
  const size = () => {
    ...
    camera.aspect = w / h;
    // On a narrow canvas (phone, portrait) the horizontal field of view is the
    // tight one, so back the camera off until the whole sphere fits the width.
    camera.position.z = 3.7 * Math.max(1, 1.1 / camera.aspect);
  ...
  function tick(now) {
    ...
    // The terminator sweeps back and forth across the face the camera sees, one
    // round trip per `dayLength` seconds. A sun that circled the whole planet
    // spent a third of every cycle behind it, leaving the visible side in night
    // — a black ball. Capping the swing at SWING keeps a lit region in view.
    const sa = RM.matches ? .6 : Math.sin(t * (Math.PI * 2 / dayLength)) * SWING;
    sun.set(Math.sin(sa), .22, Math.cos(sa)).normalize();
  ...
  /* Mobile browsers drop the WebGL context under memory pressure or when the tab
     is backgrounded, which leaves a black canvas. Let the browser restore it
     (preventDefault opts in) and repaint from the current tokens when it does. */
  cv.addEventListener('webglcontextlost', e => e.preventDefault());
  cv.addEventListener('webglcontextrestored', () => { size(); api.retheme() });
